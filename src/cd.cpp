...
const int MAX_NUMBER_OF_OBJECTS = 1000;
...
void checkCollisionWithWalls(int i){
  ...
  double centrex = centres[0][0][i].item<double>();
  double centrey = centres[1][0][i].item<double>();
  double rad = radii[0][i].item<double>();
  ...
  if (centrex - rad <= 0) velocity[0][0][i] = abs(velocity[0][0][i]);
  if (centrex + rad >= WINDOW_WIDTH) velocity[0][0][i] = -abs(velocity[0][0][i]);
  if (centrey - rad <= 0) velocity[1][0][i] = abs(velocity[1][0][i]); 
  if (centrey + rad >= WINDOW_HEIGHT) velocity[1][0][i] = -abs(velocity[1][0][i]);
}
...
void drawCircle (double radius , double centreX , double centreY , int cred , int cblue , int cgreen){
  double theta;
...
void display (){
  ...
  for (int i = 0; i < numObjects; i++){
    drawCircle (radii[0][i].item<double>() , centres[0][0][i].item<double>() , centres[1][0][i].item<double>()
    , (i == 0 ? 1 : 0) , (i == 0 ? 0 : 1) , (i == 0 ? 0 : 0));
  }
  glFlush();
  glutSwapBuffers();
...
void drawObject (double centreX, double centreY , double velocityX, double velocityY , double rs){
  /* All the different circles of the screen are drawn initially */
    for (int r = 3; r <= 50; r += 15){
      for (int i = 0; i <= 360; i += 30){
          double theta = 1.00 * i * PI / 180;
          double xc = centreX + r * cos(theta) , yc = centreY + r * sin(theta);
